    ...
    public function pass()
    {
        ...
        // } else {
            $data = $this->_vali([
                ...
                'confirm_password.confirm:new_password' => '两次输入的密码不一致！',
            ]);
            $user = SystemUser::mk()->findOrEmpty($data['id']);
            if ($user->isExists() && $user->save(['password' => md5($data['new_password'])])) {
                // 修改密码同步事件处理
                $this->app->event->trigger('PluginAdminChangePassword', [
                    'uuid' => $data['id'], 'pass' => $data['new_password']
                ]);
    ...
    protected function _form_filter(array &$data)
    {
        if ($this->request->isPost()) {
            ...
            if (empty($data['id'])) {
                ...
                // Always unset these fields after processing
